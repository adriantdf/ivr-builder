// document.onload = (function (d3, saveAs, Blob, undefined) {
  ...
  var GraphCreator = function (svg, nodes, edges) {
    ...
    var dragSvg = d3.behavior.zoom()
      .scaleExtent([0.5, 5]) // restrict zoom levels
      .on("zoom", function () {
  ...
  GraphCreator.prototype.replaceSelectNode = function (d3Node, nodeData) {
    ...
    // on click element, open sidebar and send node data
    sidebar.classList.add("open");
    getNodeData(thisGraph);
  };
  ...
  GraphCreator.prototype.updateGraph = function () {
    ...
    paths.style('marker-end', 'url(#end-arrow)')
      ...
      .attr("d", function (d) {
        ...
        const startY = d.source.y - 51;

        const endX = d.target.x;
        const endY = d.target.y + 51;
    ...
    newGs.classed(consts.circleGClass, true)  
      ...
      .on("mouseout", function (d) {
        // d3.select(this).select('.delete').remove();
  ...
  function getNodeData(thisGraph) {
    d3.event.preventDefault();
    const sidebarDeleteBtn = document.getElementById('deleteNode');

    // create delete btn click event
    sidebarDeleteBtn.onclick = () => {
      deleteNode(thisGraph);
    };
  }
...
function deleteNode(thisGraph) {
  const selectedEdge = thisGraph.state.selectedEdge;
  if (thisGraph.state.selectedNode) {
    thisGraph.nodes.splice(thisGraph.nodes.indexOf(thisGraph.state.selectedNode), 1);
    thisGraph.spliceLinksForNode(thisGraph.state.selectedNode);
    thisGraph.state.selectedNode = null;
